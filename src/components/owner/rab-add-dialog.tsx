...
export function RabAddDialog({ projectId }: RabAddDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (budget > MAX_MONEY) {
        throw new Error(
          'Total Budget (RAB) terlalu besar (maksimum Rp 999.999.999.999.999,99)',
        );
      }
      ...
      if (unitPrice > MAX_MONEY) {
        throw new Error(
          'Harga Satuan (RAB) terlalu besar (maksimum Rp 999.999.999.999.999,99)',
        );
      }
      ...
      if (realUnitPrice > MAX_MONEY) {
        throw new Error(
          'Harga Satuan (Realistis) terlalu besar (maksimum Rp 999.999.999.999.999,99)',
        );
      }
      ...
      if (realAmount > MAX_MONEY) {
        throw new Error(
          'Total Realistis terlalu besar (maksimum Rp 999.999.999.999.999,99)',
        );
      }
    ...
    <Dialog open={open} onOpenChange={(v) => !loading && setOpen(v)}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 border p-4 rounded-md bg-muted/20">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateCalculations('unitPrice', e.target.value)
                }
                disabled={loading}
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 border p-4 rounded-md bg-muted/20">
            <div className="space-y-2">
              <Label htmlFor="rab-realUnitPrice">
                Harga Satuan (Realistis)
              </Label>
              <Input
                ...
                onChange={(e) =>
                  updateCalculations('realUnitPrice', e.target.value)
                }
                disabled={loading}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateCalculations('realAmount', e.target.value)
                }
                disabled={loading}
              />
            </div>
