          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
            <Card className="flex flex-row items-center p-6 gap-4 border border-border/60 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.03)]">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-medium text-muted-foreground truncate">Total Cash In</p>
                <h3 className="text-xl font-bold truncate" title={`Rp ${totalIn.toLocaleString("id-ID")}`}>
            ...
            <Card className="flex flex-row items-center p-6 gap-4 border border-border/60 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.03)]">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-medium text-muted-foreground truncate">Total Cash Out</p>
                <h3 className="text-xl font-bold truncate" title={`Rp ${totalOut.toLocaleString("id-ID")}`}>
            ...
            <Card className="flex flex-row items-center p-6 gap-4 border border-border/60 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.03)]">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-medium text-muted-foreground truncate">Saldo Akhir</p>
                <h3 className={`text-xl font-bold truncate ${balance < 0 ? "text-red-600" : ""}`} title={`Rp ${balance.toLocaleString("id-ID")}`}>
            ...
            <Card className="flex flex-row items-center p-6 gap-4 border border-border/60 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.03)]">
              <div className={`flex items-center justify-center w-12 h-12 rounded-full shrink-0 ${
                status === "AMAN" ? "bg-emerald-100 text-emerald-600" : 
                status === "WASPADA" ? "bg-amber-100 text-amber-600" : 
                "bg-red-100 text-red-600"
              }`}>
                <AlertTriangle className="h-6 w-6" />
              </div>
              <div className="overflow-hidden">
                <p className="text-sm font-medium text-muted-foreground truncate">Status Keuangan</p>
                <h3 className={`text-xl font-bold truncate ${
                  status === "AMAN" ? "text-emerald-600" : 
                  status === "WASPADA" ? "text-amber-600" : 
                  "text-red-600"
                }`}>
                  {status}
                </h3>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          Spent: Rp {Number(project.spent).toLocaleString("id-ID")}
                        </p>
                      </div>
                    </div>
                  ))}
